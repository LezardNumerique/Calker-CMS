...
class Css
	...
	public static function parse($css)
		...
		$css = str_replace("../", "../../", $css); // Images for cache
	...
	public function add($items)
	{
		...
		if (is_array($items)) {
            foreach ($items as $item) {
                if (!in_array($item, $this->css))
                {
					if(isset($this->obj->system->cache_css) && $this->obj->system->cache_css == 1)
					{
						$css = $item.'.css';
						$this->_make($css, APPPATH.'views/'.$this->obj->config->item('theme_admin').'/css', $item.'.css');
						$css = APPPATH.'views/'.$this->obj->config->item('theme_admin').'/css/.'.$this->obj->config->item('cache_folder').'/'.$item.'.css';
					}
					else
					{
						$css = APPPATH.'views/'.$this->obj->config->item('theme_admin').'/css/'.$item.'.css';
					}
                    $this->css[] = $css;
                }
            }
        ...
        {
            if (!in_array($items, $this->css))
            {
				if($this->obj->system->cache_css == 1)
				{
					$css = $items.'.css';
					$this->_make($css, APPPATH.'modules/'.$items.'/css', $items.'.css');
					$css = APPPATH.'modules/'.$items.'/css/.'.$this->obj->config->item('cache_folder').'/'.$items.'.css';
				}
				else
				{
					$css = APPPATH.'modules/'.$items.'/css/'.$items.'.css';
				}
				$this->css[] = $css;
            }
        }

        //pre_affiche($this->css);

	}

	private function _make($item, $folder, $file)
	{
		if (!is_file($folder.'/.cache/'.$file))
		{
			if($this->obj->system->cache_css == 1)
			{
				$css = file_get_contents($folder.'/'.$file);
				$css = Css::minify($css);
				file_put_contents($folder.'/.cache/'.$file, $css);
			}
		}
	}
    ...
    {
        return 'css are: '.implode(',', $this->css);
    }
}
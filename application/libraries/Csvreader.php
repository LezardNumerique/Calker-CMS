...
class Csvreader {
     ...
     * @return    array
     */
    function parse_file($p_Filepath, $p_NamedFields = true, $separator = ';', $enclosure = '"') {
        $content = false;
        $file = fopen($p_Filepath, 'r');
        if($p_NamedFields) {
            $this->fields = fgetcsv($file, $this->max_row_size, $separator, $enclosure);
        }
        while( ($row = fgetcsv($file, $this->max_row_size, $separator, $enclosure)) != false ) {

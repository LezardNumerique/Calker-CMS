...
<div id="dialog"></div>
<div id="main">
	<h2><?php echo ($categorie['id'])? $this->lang->line('title_edit_categories').' : '.$categorie['title'] : $this->lang->line('title_create_categories');?></h2>
	<?php echo form_open(($categorie['id']) ? $this->config->item('admin_folder').'/'.$module.'/categoriesEdit/'.$categorie['id'] : $this->config->item('admin_folder').'/'.$module.'/categoriesCreate', array('enctype' => 'multipart/form-data', 'class' => (!$categorie['id'] ? 'uri_autocomplete' : '')));?>
		...
		<div id="tabs">
			<ul>
				...
				<li><a href="#four"><?php echo $this->lang->line('menu_image');?></a></li>
			</ul>
			<fieldset>
				<div id="one">
					<label for="title"><?php echo $this->lang->line('label_title');?></label>
					<input name="title" id="title" type="text" value="<?php if($this->input->post('title')) echo $this->input->post('title');else echo $categorie['title'];?>" class="input_text" maxlength="64"/>
					...
					<select name="parent_id" id="parent_id" class="input_select" />
						...
						<option value="<?php echo $parent['id']?>" <?php echo ($categorie['parent_id'] == $parent['id'] || (isset($categories_id) && $categories_id == $parent['id'])) ? "selected" : "";?>><?php echo ($parent['level'] > 0) ? "|".str_repeat("__", $parent['level']): ""?> <?php echo $parent['title'];?></option>
						<?php endforeach;?>
						<?php endif;?>
					</select>
					<label for="active"><?php echo $this->lang->line('label_status');?></label>
					<select name="active" class="input_select" id="active">
						<option value="1" <?php echo ($categorie['active'] == '1') ? 'selected="selected"' : '';?>><?php echo $this->lang->line('option_activate')?></option>
						<option value="0" <?php echo ($categorie['active'] == '0') ? 'selected="selected"' : '';?>><?php echo $this->lang->line('option_desactivate')?></option>
					</select>
				</div>
				<div id="two">
					<textarea name="body" id="body" class="input_textarea"><?php echo $categorie['body'];?></textarea>
				</div>
				<div id="three">
				...
				<div id="four">
					...
					<?php else :?>
					<img src="<?php echo site_url($this->config->item('medias_folder').'/images/120x80/default.jpg');?>" alt="<?php echo $categorie['title'];?>" width="120" height="80"/>
					<?php endif;?>
					<br />
					<br />

...
jQuery(function($) {
	...
	calker.init = function() {
		...
		// SS navigation
		$(".dp_navigation_sub").mouseenter(
			function () {
				$(".navigation_sub").hide();
				$(this).children('ul').show();
				$(".dp_navigation_sub").mouseleave(
					function () {
						$(".navigation_sub").hide();
						$('#dpm_'+DPM_ID).show();
					}
		...
		/*
		$('a.tooltip, .ui-datepicker-trigger').tooltip({
			track: true,
			delay: 0,
			fixPNG: true,
			showURL: false,
		...
		$("input.input_file").filestyle({
			image: BASE_URI+"application/views/"+ADMIN_THEME+"/img/upload_file.gif",
			imageheight : 40,
			imagewidth : 78,
			width : 240
		});
